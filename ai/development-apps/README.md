...
私たちの組織は、ゲームパブリッシングに必要なさまざまな機能を開発・運用しています。最近、業務効率を高めるためにLLM（large language model）アプリケーションを開発し、LLMOpsシステムの構築プロジェクトを行った。

### LLMアプリケーションの開発プロセス

- **高性能LLMの利用**: OpenAIのGPTやAnthropicのClaudeなどのモデルを活用し、コスト効率を考慮して商用モデルを使用。
- **プロンプトの構造化**: モデルが理解しやすいようにプロンプトを構造化し、性能を向上。
- **インコンテキスト学習**: 追加情報を用いてモデルが新しいデータに適切に回答できるようにする。
- **RAG（Retrieval Augmented Generation）**: 外部データベースから情報をリアルタイムで注入し、モデルの回答を最適化。

### LLMアプリケーション開発時に直面する課題

- **プロンプトとコードの依存関係**: 自然言語プロンプトと静的なPythonコードが同時に存在し、修正が難しい。
- **ボイラープレートコードの生成**: RAGのためのベクトルデータベースとプロンプトチェーンが複雑で、デバッグが困難。

### 課題を解決するための3つのアプローチ

1. **Prompt Storeの開発**: プロンプトを即座に実行・共有できる環境を構築。
   - プロンプトの作成・編集機能
   - Playground機能でのテスト
   - 再利用機能の提供

2. **ビジュアルスクリプティングの導入**: Langflowを利用して再利用可能なコンポーネントを可視化。
   - ドラッグアンドドロップでのアプリケーション作成
   - 簡単なカスタマイズと新機能の追加

3. **簡単なデプロイシステムの構築**: LLMアプリケーションの結果を素早く確認できるようにする。
   - デプロイボタンで簡単にAPIインスタンスを作成

### 3つのアプローチの詳細

#### 1. Prompt Storeの開発

- **プロンプトの作成と編集**: ユーザーがプロンプトに名前を付けて保存し、タグを追加可能。
- **Playground機能**: 様々な環境でプロンプトをテストできる。

#### 2. Langflowを利用したビジュアルスクリプティング

- **Langflowの導入**: ドラッグアンドドロップでアプリケーションを作成。
- **カスタム機能の作成**: 自由にカスタマイズ可能なコンポーネントを実装。

#### 3. 簡単なデプロイシステムの構築

- **デプロイメント構造**: Kubernetes APIを使用してデプロイメントを管理。

### 結果

これらのアプローチを導入した結果、職種に関係なく多くのメンバーがLLMアプリケーションを自分で作成し、デプロイできる環境が整った。
...
### 大量のデータをAIエージェントで検索性能を上げる

AIエージェントの仕組み

1. **文書のインデックス化**:
    大量のドキュメントを取り込み、テキストデータに変換し、小さな単位に分割してインデックス化する
2. **ベクトル化**:
    各テキスト単位をAIモデルを使用してベクトル（数値表現）に変換する。
3. **検索処理**:
    ユーザーのクエリをキーワードに変換し、意味的に最も類似したテキスト単位を検索する。
4. **AIモデルによる回答生成**:
    検索結果と元のクエリを生成AIモデル（例：ChatGPT）に送信し、回答を生成する。
5. **結果の表示**:
   生成された回答と、使用された元のテキスト（出典）をユーザーに表示する
