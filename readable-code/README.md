...
- do/whileループを避ける
  - do-statementはエラーや混乱の原因になることが多い
- 関数から早く返す
  - 関数の出口を1つにしちあというのは、何らかのクリーンアップコードを確実に実行したい場合
  - 言語ごとにクリーンアップコードのイディオムがある。(golangならdefer)
- ネストを浅くする
  - 早めに返してネストを削除する
- 実行の流れを追えるか？
...
- テスト駆動開発
  - TDDは本物をコードを書く前にテストを書くというプログラミング手法で、コードを書いてからテストを書くよりもコードの品質が飛躍的に向上すると言われている。
